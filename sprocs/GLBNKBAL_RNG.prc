rem ----------------------------------------------------------------------------
rem Program: GLBNKBAL_RNG.prc     (orig: DASHBD_GLBANKBALS-RING.prc)
...
rem              the "Bank Account Balances" RING widget
rem
rem              Includes a minimum amount filter.
rem
rem    ****  NOTE: Currently limited to Current Year or Prior Year full year totals. 
rem    ****        Code is written with conditionals for possible 
...
rem --- Get the IN parameters used by the procedure
	...
	include_type$ = sp!.getParameter("INCLUDE_TYPE"); rem As listed below; used to access requested GL Record ID(s)
													  ...
													  rem B = Not used in order to align with DashBoard_GLInc_YrTots
													  rem C = Prior Year Actual full totals (no period/date consideration)
	if pos(include_type$="AC")=0
		include_type$="A"; rem default to Current Year Actual
	endif
	
	min_amt = sp!.getParameter("MIN_AMT")
...
rem --- create the in memory recordset for return
	...
	dataTemplate$ = "ACCOUNT:C(1*),TOTAL:C(7*)"
