rem ----------------------------------------------------------------------------
rem Program: CUST_PRINTPAYMENTS.prc
rem Description: Stored Procedure to create a jasper-based payment history
...
rem --- create the in memory recordset for return
	...
	dataTemplate$ = dataTemplate$ + "invoice_date:C(10),ar_inv_no:C(7),invoice_amt:C(1*),trans_amt:C(1*),"
	dataTemplate$ = DataTemplate$ + "invBalance:C(1*),"
...
rem --- get payment_url for the cash_rec_cd used for customer payments (only one record allowed at present)
	...
	read record(art01, key = firm_id$ + "  " + customer$ + ar_inv_no$, dom=*next)
...
rem --- main loop
	while 1
		...
		if art01.ar_inv_no$ <> ar_inv_no$ then break
