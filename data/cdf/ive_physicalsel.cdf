...
[[IVE_PHYSICALSEL.ASVA]]
rem --- Check for values in warehouse and cutoff date

	whse$   = callpoint!.getColumnData("IVE_PHYSICALSEL.WAREHOUSE_ID")
	cutoff$ = callpoint!.getColumnData("IVE_PHYSICALSEL.CUTOFF_DATE")
	...
	if cutoff$ = "" then
		callpoint!.setMessage("IV_NEED_CUTOFF")
		callpoint!.setStatus("ABORT")
		goto asva_end
	endif
...
rem --- Roll thru grid rows, saving the pending action of checked records
	...
	callpoint!.setDevObject("IVE_PHYSICALSEL.SELECTED_ALL", selected_all)

asva_end:

[[IVE_PHYSICALSEL.AWIN]]
rem --- Inits
...
[[IVE_PHYSICALSEL.WAREHOUSE_ID.AVAL]]
...
rem ==========================================================================
	...
	while more
		...
		if physcode_rec.phys_inv_sts$ = "0" and 
:			(physcode_rec.pending_action$ = "0" or physcode_rec.pending_action$ = "1") 
:		then 
			...
			if physcode_rec.pending_action$ = "1" then 
				user_tpl.cutoff_default$ = physcode_rec.cutoff_date$
