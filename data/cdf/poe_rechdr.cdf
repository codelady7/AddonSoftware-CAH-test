...
rem --- Open Files
	...
	open_tables$[18]="APM_VENDADDR",open_opts$[18]="OTA"
...
get_dropship_order_lines: rem --- Build list of dropship lines in linked SO
	poe_linked_dev=fnget_dev("POE_LINKED")
	...
	read(poe_linked_dev,key=firm_id$+po_no$,dom=*next)
	while 1
		poe_linked_key$=key(poe_linked_dev,end=*break)
		if pos(firm_id$+po_no$=poe_linked_key$)<>1 then break
		readrecord(poe_linked_dev)poe_linked$
		redim ope_orddet$
		ope_orddet_key$=firm_id$+"  "+poe_linked.customer_id$+poe_linked.order_no$+ope_orddet.ar_inv_no$+poe_linked.opedet_seq_ref$
		readrecord(ope_orddet_dev,key=ope_orddet_key$,dom=*continue)ope_orddet$
