...
rem --- Create totals
	...
	if cvs(callpoint!.getColumnData("SAM_WAREHSE.YEAR"),3)<>""
...
[[SAM_WAREHSE.AOPT-SUMM]]
rem --- Calculate and display summary info
	tcst=0
	tqty=0
	tsls=0
	year$=callpoint!.getColumnData("SAM_WAREHSE.YEAR")
	lyear$=str(num(year$)-1:"0000")
	trip_key$=firm_id$+year$+callpoint!.getColumnData("SAM_WAREHSE.WAREHOUSE_ID")
	ltrip_key$=firm_id$+lyear$+callpoint!.getColumnData("SAM_WAREHSE.WAREHOUSE_ID")
	prod_type$=callpoint!.getColumnData("SAM_WAREHSE.PRODUCT_TYPE")
	item_no$=callpoint!.getColumnData("SAM_WAREHSE.ITEM_ID")
	if cvs(prod_type$,2)<>"" 
		trip_key$=trip_key$+prod_type$
		ltrip_key$=ltrip_key$+prod_type$
	else
		callpoint!.setColumnData("SAM_WAREHSE.PRODUCT_TYPE","")
	endif
	callpoint!.setColumnData("SAM_WAREHSE.ITEM_ID","")
...
rem --- Start progress meter
	...
	sam_dev=	fnget_dev("SAM_WAREHSE")
	dim sam_tpl$:fnget_tpl$("SAM_WAREHSE")
...
rem --- Now display all of these things and disable key fields
	for x=1 to 13
		callpoint!.setColumnData("SAM_WAREHSE.TOTAL_SALES_"+str(x:"00"),str(sales[x]))
		callpoint!.setColumnData("SAM_WAREHSE.TOTAL_COST_"+str(x:"00"),str(cost[x]))
		callpoint!.setColumnData("SAM_WAREHSE.QTY_SHIPPED_"+str(x:"00"),str(qty[x]))
	next x
	...
	callpoint!.setColumnEnabled("SAM_WAREHSE.YEAR",0)
	callpoint!.setColumnEnabled("SAM_WAREHSE.WAREHOUSE_ID",0)
	callpoint!.setColumnEnabled("SAM_WAREHSE.PRODUCT_TYPE",0)
	callpoint!.setColumnEnabled("SAM_WAREHSE.ITEM_ID",0)
...
rem --- Enable key fields
	...
	callpoint!.setColumnEnabled("SAM_WAREHSE.PRODUCT_TYPE",1)
	callpoint!.setColumnEnabled("SAM_WAREHSE.ITEM_ID",1)
...
rem --- Use current selections for initiating next record
	year$=callpoint!.getColumnData("SAM_WAREHSE.YEAR")
	warehouse_id$=callpoint!.getColumnData("SAM_WAREHSE.WAREHOUSE_ID")
	product_type$=callpoint!.getColumnData("SAM_WAREHSE.PRODUCT_TYPE")
	item_id$=callpoint!.getColumnData("SAM_WAREHSE.ITEM_ID")
	sam_dev=fnget_dev("SAM_WAREHSE")
	read(sam_dev,key=firm_id$+year$+warehouse_id$+product_type$+item_id$,dom=*next)
...
rem --- Use current selections for initiating previous record
	...
	item_id$=callpoint!.getColumnData("SAM_WAREHSE.ITEM_ID")
	sam_dev=fnget_dev("SAM_WAREHSE")
	read(sam_dev,key=firm_id$+year$+warehouse_id$+product_type$+item_id$,dir=0,dom=*next)
...
rem --- Check for parameter record
	...
	if sas01a.by_whse$<>"Y"
		msg_id$="INVALID_SA"
		dim msg_tokens$[1]
		msg_tokens$[1]=Translate!.getTranslation("AON_WAREHOUSE")
		gosub disp_message
		bbjAPI!=bbjAPI()
...
rem --- Use fiscal period and abbreviation for table row label
	...
	for i=1 to 13
		...
		ctlContext=num(callpoint!.getTableColumnAttribute("SAM_WAREHSE.QTY_SHIPPED_"+period$,"CTLC"))
		ctlID=num(callpoint!.getTableColumnAttribute("SAM_WAREHSE.QTY_SHIPPED_"+period$,"CTLI"))
...
rem ========================================================
	...
	lyear$=str(num(year$)-1:"0000")
	whse$=callpoint!.getColumnData("SAM_WAREHSE.WAREHOUSE_ID")
	prod$=callpoint!.getColumnData("SAM_WAREHSE.PRODUCT_TYPE")
	item$=callpoint!.getColumnData("SAM_WAREHSE.ITEM_ID")
	ltrip_key$=firm_id$+lyear$+whse$+prod$+item$
	sam_dev=fnget_dev("@SAM_WAREHSE")
	dim sam_tpl$:fnget_tpl$("@SAM_WAREHSE")
	dim qty[13],cost[13],sales[13]
	hshThisYear! = new java.util.HashMap()
	...
	tsls=0
	For x=1 to 13
		cst=num(callpoint!.getColumnData("SAM_WAREHSE.TOTAL_COST_"+str(x:"00")))
		qty=num(callpoint!.getColumnData("SAM_WAREHSE.QTY_SHIPPED_"+str(x:"00")))
		sls=num(callpoint!.getColumnData("SAM_WAREHSE.TOTAL_SALES_"+str(x:"00")))
		tcst=tcst+cst
		tqty=tqty+qty
		tsls=tsls+sls
...
rem ========================================================
	...
	ctl1!=callpoint!.getControl("SAM_WAREHSE.YEAR")
...
rem --- create StackedBarChartEmbeddedWidget to show sales and cost for selected and prior years
	...
	widgetName$ = "SACust"
	title$ = "Sales Analysis by Warehouse"
	chartTitle$ = ""
	domainTitle$ = ""
