...
rem --- Enable/disable Sales Order fields depending on if Order Processing is installed
	...
	if op$<>"Y" then
		...
		callpoint!.setColumnEnabled("POR_OPENREQSN.ORDER_NO_1",0)
		callpoint!.setColumnEnabled("POR_OPENREQSN.ORDER_NO_2",0)
	else
		rem --- OP installed, enable Sales Order fields
		callpoint!.setColumnEnabled("POR_OPENREQSN.ORDER_NO_1",1)
		callpoint!.setColumnEnabled("POR_OPENREQSN.ORDER_NO_2",1)
	endif

[[POR_OPENREQSN.ORDER_NO.BINQ]]
rem --- Sales Order Lookup
	...
	if cvs(optInvHdr_key$,2)<>"" then 
		ctrl_1!=callpoint!.getControl("POR_OPENREQSN.ORDER_NO_1")
		ctrl_2!=callpoint!.getControl("POR_OPENREQSN.ORDER_NO_2")
		if num(callpoint!.getControlID())=ctrl_1!.getID() then
			callpoint!.setColumnData("POR_OPENREQSN.ORDER_NO_1",optInvHdr_key.order_no$,1)
		endif
		if num(callpoint!.getControlID())=ctrl_2!.getID() then
			callpoint!.setColumnData("POR_OPENREQSN.ORDER_NO_2",optInvHdr_key.order_no$,1)
		endif
	endif
