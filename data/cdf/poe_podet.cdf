...
rem --- Inventory Item/Whse Lookup
	...
	filter_defs$[2,1]="='"+callpoint!.getColumnData("POE_PODET.WAREHOUSE_ID")+"'"
	filter_defs$[2,2]=""
	...
	if cvs(ivmItemWhse_key$,2)<>"" and ivmItemWhse_key.item_id$<>callpoint!.getColumnData("POE_PODET.ITEM_ID") then 
		callpoint!.setColumnData("POE_PODET.ITEM_ID",ivmItemWhse_key.item_id$,1)
		callpoint!.setStatus("MODIFIED")
		callpoint!.setFocus(num(callpoint!.getValidationRow()),"POE_PODET.ITEM_ID",1)
	endif
...
rem --- Don't allow new transactions if WO is being closed complete. Warn existing transactions.
	...
	if closeComplete then
		...
		msg_tokens$[1]=wo_no$
		gosub disp_message
		if wo_no$<>callpoint!.getColumnData("POE_PODET.WO_NO") then
...
rem ==========================================================================
	...
	if callpoint!.getVariableName()="POE_PODET.ITEM_ID" then
		item_id$=callpoint!.getUserInput()
		if item_id$<>callpoint!.getColumnData("POE_PODET.ITEM_ID") or callpoint!.getGridRowNewStatus(callpoint!.getValidationRow())="Y"
			change_flag=1
		 endif
	else
		item_id$=callpoint!.getColumnData("POE_PODET.ITEM_ID")
	...
	if callpoint!.getVariableName()="POE_PODET.WAREHOUSE_ID" then
		...
		if whse$<>callpoint!.getColumnData("POE_PODET.WAREHOUSE_ID") or callpoint!.getGridRowNewStatus(callpoint!.getValidationRow())="Y"
			change_flag=1
		endif
	else
		whse$=callpoint!.getColumnData("POE_PODET.WAREHOUSE_ID")
