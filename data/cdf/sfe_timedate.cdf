...
if num(stbl("+BATCH_NO"),err=*next)<>0
	...
	primaryKey$=callpoint!.getColumnData("SFE_TIMEDATE.FIRM_ID")+
:		callpoint!.getColumnData("SFE_TIMEDATE.TRANS_DATE")+
:		callpoint!.getColumnData("SFE_TIMEDATE.EMPLOYEE_NO")
...
rem --- Get SF parameters
	...
	rem --- Must set DevObject "pr" in BFMC instead of BSHO because grid BDGC executed before BSHO
...
rem --- Initialize trans_date
	if cvs(callpoint!.getColumnData("SFE_TIMEDATE.TRANS_DATE"),2)="" then 
		callpoint!.setColumnData("SFE_TIMEDATE.TRANS_DATE",stbl("+SYSTEM_DATE"),1)
	endif
