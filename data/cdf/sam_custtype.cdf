...
rem --- Create totals
	...
	if cvs(callpoint!.getColumnData("SAM_CUSTTYPE.YEAR"),3)<>""
...
rem --- Enable/Disable Summary button
	...
	cust_type$=callpoint!.getColumnData("SAM_CUSTTYPE.CUSTOMER_TYPE")
	prod_type$=callpoint!.getColumnData("SAM_CUSTTYPE.PRODUCT_TYPE")
	item_no$=callpoint!.getColumnData("SAM_CUSTTYPE.ITEM_ID")
	gosub summ_button

[[SAM_CUSTTYPE.AOPT-SUMM]]
rem --- Calculate and display summary info
	tcst=0
	tqty=0
	tsls=0
	year$=callpoint!.getColumnData("SAM_CUSTTYPE.YEAR")
	lyear$=str(num(year$)-1:"0000")
	trip_key$=firm_id$+year$+callpoint!.getColumnData("SAM_CUSTTYPE.CUSTOMER_TYPE")
	ltrip_key$=firm_id$+lyear$+callpoint!.getColumnData("SAM_CUSTTYPE.CUSTOMER_TYPE")
	prod_type$=callpoint!.getColumnData("SAM_CUSTTYPE.PRODUCT_TYPE")
	item_no$=callpoint!.getColumnData("SAM_CUSTTYPE.ITEM_ID")
	if cvs(prod_type$,2)<>"" 
		...
		callpoint!.setColumnData("SAM_CUSTTYPE.PRODUCT_TYPE","")
	endif
	callpoint!.setColumnData("SAM_CUSTTYPE.ITEM_ID","")
...
rem --- Start progress meter
	...
	sam_dev=	fnget_dev("SAM_CUSTTYPE")
	dim sam_tpl$:fnget_tpl$("SAM_CUSTTYPE")
...
rem --- Now display all of these things and disable key fields
	for x=1 to 13
		callpoint!.setColumnData("SAM_CUSTTYPE.TOTAL_SALES_"+str(x:"00"),str(sales[x]))
		callpoint!.setColumnData("SAM_CUSTTYPE.TOTAL_COST_"+str(x:"00"),str(cost[x]))
		callpoint!.setColumnData("SAM_CUSTTYPE.QTY_SHIPPED_"+str(x:"00"),str(qty[x]))
	next x
	...
	callpoint!.setColumnEnabled("SAM_CUSTTYPE.YEAR",0)
	callpoint!.setColumnEnabled("SAM_CUSTTYPE.CUSTOMER_TYPE",0)
	callpoint!.setColumnEnabled("SAM_CUSTTYPE.PRODUCT_TYPE",0)
	callpoint!.setColumnEnabled("SAM_CUSTTYPE.ITEM_ID",0)
...
rem --- Enable key fields
	callpoint!.setColumnEnabled("SAM_CUSTTYPE.YEAR",1)
	callpoint!.setColumnEnabled("SAM_CUSTTYPE.CUSTOMER_ID",1)
	callpoint!.setColumnEnabled("SAM_CUSTTYPE.PRODUCT_TYPE",1)
	callpoint!.setColumnEnabled("SAM_CUSTTYPE.ITEM_ID",1)
...
rem --- Use current selections for initiating next record
	year$=callpoint!.getColumnData("SAM_CUSTTYPE.YEAR")
	customer_type$=callpoint!.getColumnData("SAM_CUSTTYPE.CUSTOMER_TYPE")
	product_type$=callpoint!.getColumnData("SAM_CUSTTYPE.PRODUCT_TYPE")
	item_id$=callpoint!.getColumnData("SAM_CUSTTYPE.ITEM_ID")
	sam_dev=fnget_dev("SAM_CUSTTYPE")
	read(sam_dev,key=firm_id$+year$+customer_type$+product_type$+item_id$,dom=*next)
...
rem --- Use current selections for initiating previous record
	...
	item_id$=callpoint!.getColumnData("SAM_CUSTTYPE.ITEM_ID")
	sam_dev=fnget_dev("SAM_CUSTTYPE")
	read(sam_dev,key=firm_id$+year$+customer_type$+product_type$+item_id$,dir=0,dom=*next)
...
rem --- Check for parameter record
	...
	open_tables$[2]="SAM_CUSTTYPE",open_opts$[2]="OTA@"
...
[[SAM_CUSTTYPE.CUSTOMER_TYPE.AVAL]]
rem --- Enable/Disable Summary button
	cust_type$=callpoint!.getUserInput()
	prod_type$=callpoint!.getColumnData("SAM_CUSTTYPE.PRODUCT_TYPE")
	item_no$=callpoint!.getColumnData("SAM_CUSTTYPE.ITEM_ID")
	gosub summ_button

[[SAM_CUSTTYPE.ITEM_ID.AINV]]
rem --- Item synonym processing

	call stbl("+DIR_PGM")+"ivc_itemsyn.aon::option_entry"

[[SAM_CUSTTYPE.ITEM_ID.AVAL]]
rem --- Enable/Disable Summary button
	cust_type$=callpoint!.getColumnData("SAM_CUSTTYPE.CUSTOMER_TYPE")
	prod_type$=callpoint!.getColumnData("SAM_CUSTTYPE.PRODUCT_TYPE")
...
[[SAM_CUSTTYPE.PRODUCT_TYPE.AVAL]]
rem --- Enable/Disable Summary button
	cust_type$=callpoint!.getColumnData("SAM_CUSTTYPE.CUSTOMER_TYPE")
...
rem --- Use fiscal period and abbreviation for table row label
	...
	for i=1 to 13
		...
		ctlContext=num(callpoint!.getTableColumnAttribute("SAM_CUSTTYPE.QTY_SHIPPED_"+period$,"CTLC"))
		ctlID=num(callpoint!.getTableColumnAttribute("SAM_CUSTTYPE.QTY_SHIPPED_"+period$,"CTLI"))
...
rem ========================================================
	...
	lyear$=str(num(year$)-1:"0000")
	cust_type$=callpoint!.getColumnData("SAM_CUSTTYPE.CUSTOMER_TYPE")
	prod$=callpoint!.getColumnData("SAM_CUSTTYPE.PRODUCT_TYPE")
	item$=callpoint!.getColumnData("SAM_CUSTTYPE.ITEM_ID")
	ltrip_key$=firm_id$+lyear$+cust_type$+prod$+item$
	sam_dev=fnget_dev("@SAM_CUSTTYPE")
	dim sam_tpl$:fnget_tpl$("@SAM_CUSTTYPE")
	dim qty[13],cost[13],sales[13]	
	hshThisYear! = new java.util.HashMap()
	...
	tsls=0
	For x=1 to 13
		cst=num(callpoint!.getColumnData("SAM_CUSTTYPE.TOTAL_COST_"+str(x:"00")))
		qty=num(callpoint!.getColumnData("SAM_CUSTTYPE.QTY_SHIPPED_"+str(x:"00")))
		sls=num(callpoint!.getColumnData("SAM_CUSTTYPE.TOTAL_SALES_"+str(x:"00")))
		tcst=tcst+cst
		tqty=tqty+qty
		tsls=tsls+sls
...
rem ========================================================
	...
	ctl1!=callpoint!.getControl("SAM_CUSTTYPE.YEAR")
...
rem --- create StackedBarChartEmbeddedWidget to show sales and cost for selected and prior years
	...
	widgetName$ = "SACust"
	title$ = "Analysis by Customer Type"
	chartTitle$ = ""
	domainTitle$ = ""
