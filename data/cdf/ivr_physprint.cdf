...
rem --- Check for values in warehouse and cutoff date
	...
	whse$   = callpoint!.getColumnData("IVR_PHYSPRINT.WAREHOUSE_ID")
...
rem --- Roll thru grid rows, saving the pending action of checked records
	...
	callpoint!.setDevObject("IVR_PHYSPRINT.SELECTED_ALL", selected_all)
...
[[IVR_PHYSPRINT.WAREHOUSE_ID.AVAL]]
rem --- Filter grid on selected warehouse

	whse$ = callpoint!.getUserInput()

	if callpoint!.getColumnUndoData("IVR_PHYSPRINT.WAREHOUSE_ID") <> whse$ then
		user_tpl.whse_changed = 1
	endif

	gosub fill_grid

[[IVR_PHYSPRINT.<CUSTOM>]]
rem ==========================================================================
...
:		then 
			...
			if physcode_rec.pending_action$ = "2" then 
