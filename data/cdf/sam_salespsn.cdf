...
rem --- Calculate and display summary info
	...
	trip_key$=firm_id$+year$+callpoint!.getColumnData("SAM_SALESPSN.SLSPSN_CODE")
	ltrip_key$=firm_id$+lyear$+callpoint!.getColumnData("SAM_SALESPSN.SLSPSN_CODE")
	...
	if cvs(prod_type$,2)<>"" 
		...
		callpoint!.setColumnData("SAM_SALESPSN.PRODUCT_TYPE","")
	endif
	callpoint!.setColumnData("SAM_SALESPSN.ITEM_ID","")
...
rem --- Start progress meter
	...
	dim sam_tpl$:fnget_tpl$("SAM_SALESPSN")
...
rem --- Now display all of these things and disable key fields
	for x=1 to 13
		callpoint!.setColumnData("SAM_SALESPSN.TOTAL_SALES_"+str(x:"00"),str(sales[x]))
		callpoint!.setColumnData("SAM_SALESPSN.TOTAL_COST_"+str(x:"00"),str(cost[x]))
		callpoint!.setColumnData("SAM_SALESPSN.QTY_SHIPPED_"+str(x:"00"),str(qty[x]))
	next x
	...
	callpoint!.setColumnEnabled("SAM_SALESPSN.YEAR",0)
	callpoint!.setColumnEnabled("SAM_SALESPSN.SLSPSN_CODE",0)
	callpoint!.setColumnEnabled("SAM_SALESPSN.PRODUCT_TYPE",0)
	callpoint!.setColumnEnabled("SAM_SALESPSN.ITEM_ID",0)
...
rem ========================================================
	...
	year$=callpoint!.getColumnData("SAM_SALESPSN.YEAR")
	lyear$=str(num(year$)-1:"0000")
	slspsn$=callpoint!.getColumnData("SAM_SALESPSN.SLSPSN_CODE")
	prod$=callpoint!.getColumnData("SAM_SALESPSN.PRODUCT_TYPE")
	item$=callpoint!.getColumnData("SAM_SALESPSN.ITEM_ID")
	ltrip_key$=firm_id$+lyear$+slspsn$+prod$+item$
	sam_dev=fnget_dev("@SAM_SALESPSN")
	dim sam_tpl$:fnget_tpl$("@SAM_SALESPSN")
	dim qty[13],cost[13],sales[13]
	hshThisYear! = new java.util.HashMap()
	...
	tsls=0
	For x=1 to 13
		cst=num(callpoint!.getColumnData("SAM_SALESPSN.TOTAL_COST_"+str(x:"00")))
		qty=num(callpoint!.getColumnData("SAM_SALESPSN.QTY_SHIPPED_"+str(x:"00")))
		sls=num(callpoint!.getColumnData("SAM_SALESPSN.TOTAL_SALES_"+str(x:"00")))
		tcst=tcst+cst
		tqty=tqty+qty
		tsls=tsls+sls
...
rem ========================================================
	...
	ctl1!=callpoint!.getControl("SAM_SALESPSN.YEAR")
...
rem --- create StackedBarChartEmbeddedWidget to show sales and cost for selected and prior years
	...
	widgetName$ = "SACust"
	title$ = "Sales Analysis by Salesperson"
	chartTitle$ = ""
	domainTitle$ = ""
