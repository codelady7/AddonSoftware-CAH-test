...
rem --- Check for values in warehouse and cutoff date
	...
	whse$ = callpoint!.getColumnData("IVR_PHYSREG.WAREHOUSE_ID")
...
rem --- Roll thru grid rows, saving the pending action of checked records
	...
	while more 
		...
		if physcode_rec.phys_inv_sts$ <> "2" then 
			selected_all = 0
			read (physcode_dev)
			continue
		endif
...
rem --- Filter grid on selected warehouse
	...
	if callpoint!.getColumnUndoData("IVR_PHYSREG.WAREHOUSE_ID") <> whse$ then
