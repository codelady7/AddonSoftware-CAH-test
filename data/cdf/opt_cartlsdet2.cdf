...
rem --- Disable lotser_no and qty_picked when the carton has shipped so they cannot be changed
	if callpoint!.getDevObject("shipped_flag")="Y" then
		callpoint!.setColumnEnabled(callpoint!.getValidationRow(),"OPT_CARTLSDET2.LOTSER_NO",0)
		callpoint!.setColumnEnabled(callpoint!.getValidationRow(),"OPT_CARTLSDET2.QTY_PACKED",0)
	endif
	...
	if callpoint!.getDevObject("shipped_flag")="Y" then
		...
		if GridVect!.size()=1 then
			...
			for row=0 to GridVect!.size()-1
				callpoint!.setColumnEnabled(row,"OPT_CARTLSDET2.LOTSER_NO",0)
				callpoint!.setColumnEnabled(row,"OPT_CARTLSDET2.QTY_PACKED",0)
			next row
