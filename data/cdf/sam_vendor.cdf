...
rem --- Create totals
	...
	if cvs(callpoint!.getColumnData("SAM_VENDOR.YEAR"),3)<>""
...
rem --- Enable key fields
	callpoint!.setColumnEnabled("SAM_VENDOR.YEAR",1)
	callpoint!.setColumnEnabled("SAM_VENDOR.CUSTOMER_ID",1)
	callpoint!.setColumnEnabled("SAM_VENDOR.PRODUCT_TYPE",1)
	callpoint!.setColumnEnabled("SAM_VENDOR.ITEM_ID",1)
...
rem --- Use current selections for initiating next record
	year$=callpoint!.getColumnData("SAM_VENDOR.YEAR")
	vendor_id$=callpoint!.getColumnData("SAM_VENDOR.VENDOR_ID")
	product_type$=callpoint!.getColumnData("SAM_VENDOR.PRODUCT_TYPE")
	item_id$=callpoint!.getColumnData("SAM_VENDOR.ITEM_ID")
	sam_dev=fnget_dev("SAM_VENDOR")
	read(sam_dev,key=firm_id$+year$+vendor_id$+product_type$+item_id$,dom=*next)
...
rem --- Use current selections for initiating previous record
	...
	item_id$=callpoint!.getColumnData("SAM_VENDOR.ITEM_ID")
	sam_dev=fnget_dev("SAM_VENDOR")
	read(sam_dev,key=firm_id$+year$+vendor_id$+product_type$+item_id$,dir=0,dom=*next)
...
rem --- Check for parameter record
	...
	open_tables$[2]="SAM_VENDOR",open_opts$[2]="OTA@"
...
rem --- Use fiscal period and abbreviation for table row label
	...
	for i=1 to 13
		...
		ctlContext=num(callpoint!.getTableColumnAttribute("SAM_VENDOR.QTY_SHIPPED_"+period$,"CTLC"))
		ctlID=num(callpoint!.getTableColumnAttribute("SAM_VENDOR.QTY_SHIPPED_"+period$,"CTLI"))
...
rem ========================================================
	...
	tsls=0
	For x=1 to 13
		cst=num(callpoint!.getColumnData("SAM_VENDOR.TOTAL_COST_"+str(x:"00")))
		qty=num(callpoint!.getColumnData("SAM_VENDOR.QTY_SHIPPED_"+str(x:"00")))
		sls=num(callpoint!.getColumnData("SAM_VENDOR.TOTAL_SALES_"+str(x:"00")))
		tcst=tcst+cst
		tqty=tqty+qty
		tsls=tsls+sls
...
rem ========================================================
	...
	ctl1!=callpoint!.getControl("SAM_VENDOR.YEAR")
...
rem --- create StackedBarChartEmbeddedWidget to show sales and cost for selected and prior years
	...
	widgetName$ = "SACust"
	title$ = "Analysis by Stocking Vendor"
	chartTitle$ = ""
	domainTitle$ = ""
