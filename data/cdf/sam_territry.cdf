...
rem --- Create totals
	...
	if cvs(callpoint!.getColumnData("SAM_TERRITRY.YEAR"),3)<>""
...
rem --- Calculate and display summary info
	...
	trip_key$=firm_id$+year$+callpoint!.getColumnData("SAM_TERRITRY.TERRITORY")
	ltrip_key$=firm_id$+lyear$+callpoint!.getColumnData("SAM_TERRITRY.TERRITORY")
	prod_type$=callpoint!.getColumnData("SAM_TERRITRY.PRODUCT_TYPE")
	item_no$=callpoint!.getColumnData("SAM_TERRITRY.ITEM_ID")
	if cvs(prod_type$,2)<>"" 
...
rem --- Enable key fields
	...
	callpoint!.setColumnEnabled("SAM_TERRITRY.TERRITORY",1)
	callpoint!.setColumnEnabled("SAM_TERRITRY.PRODUCT_TYPE",1)
	callpoint!.setColumnEnabled("SAM_TERRITRY.ITEM_ID",1)
...
rem --- Calculate Last Year
	...
	tsls=0
	for x=1 to 13
		cst=num(callpoint!.getColumnData("SAM_TERRITRY.TOTAL_COST_"+str(x:"00")))
		qty=num(callpoint!.getColumnData("SAM_TERRITRY.QTY_SHIPPED_"+str(x:"00")))
		sls=num(callpoint!.getColumnData("SAM_TERRITRY.TOTAL_SALES_"+str(x:"00")))
		tcst=tcst+cst
		tqty=tqty+qty
		tsls=tsls+sls
...
rem ========================================================
	...
	ctl1!=callpoint!.getControl("SAM_TERRITRY.YEAR")
...
rem --- create StackedBarChartEmbeddedWidget to show sales and cost for selected and prior years
	...
	widgetName$ = "SAChart"
	title$ = "Sales Analysis by Territory"
	chartTitle$ = ""
	domainTitle$ = ""
