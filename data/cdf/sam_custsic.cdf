...
rem --- Create totals
	...
	if cvs(callpoint!.getColumnData("SAM_CUSTSIC.YEAR"),3)<>""
...
rem --- Calculate and display summary info
	...
	tqty=0
	tsls=0
	year$=callpoint!.getColumnData("SAM_CUSTSIC.YEAR")
	lyear$=str(num(year$)-1:"0000")
	trip_key$=firm_id$+year$+callpoint!.getColumnData("SAM_CUSTSIC.SIC_CODE")
	ltrip_key$=firm_id$+lyear$+callpoint!.getColumnData("SAM_CUSTSIC.SIC_CODE")
	...
	callpoint!.setColumnData("SAM_CUSTSIC.ITEM_ID","")
...
rem --- Enable key fields
	callpoint!.setColumnEnabled("SAM_CUSTSIC.YEAR",1)
	callpoint!.setColumnEnabled("SAM_CUSTSIC.CUSTOMER_ID",1)
	callpoint!.setColumnEnabled("SAM_CUSTSIC.PRODUCT_TYPE",1)
	callpoint!.setColumnEnabled("SAM_CUSTSIC.ITEM_ID",1)
...
rem --- Use current selections for initiating next record
	year$=callpoint!.getColumnData("SAM_CUSTSIC.YEAR")
	sic_code$=callpoint!.getColumnData("SAM_CUSTSIC.SIC_CODE")
	product_type$=callpoint!.getColumnData("SAM_CUSTSIC.PRODUCT_TYPE")
	item_id$=callpoint!.getColumnData("SAM_CUSTSIC.ITEM_ID")
	sam_dev=fnget_dev("SAM_CUSTSIC")
	read(sam_dev,key=firm_id$+year$+sic_code$+product_type$+item_id$,dom=*next)
...
rem --- Use current selections for initiating previous record
	...
	item_id$=callpoint!.getColumnData("SAM_CUSTSIC.ITEM_ID")
	sam_dev=fnget_dev("SAM_CUSTSIC")
	read(sam_dev,key=firm_id$+year$+sic_code$+product_type$+item_id$,dir=0,dom=*next)
...
rem --- Check for parameter record
	...
	open_tables$[2]="SAM_CUSTSIC",open_opts$[2]="OTA@"
...
rem --- Use fiscal period and abbreviation for table row label
	...
	for i=1 to 13
		...
		ctlContext=num(callpoint!.getTableColumnAttribute("SAM_CUSTSIC.QTY_SHIPPED_"+period$,"CTLC"))
		ctlID=num(callpoint!.getTableColumnAttribute("SAM_CUSTSIC.QTY_SHIPPED_"+period$,"CTLI"))
...
rem ========================================================
	...
	year$=callpoint!.getColumnData("SAM_CUSTSIC.YEAR")
	lyear$=str(num(year$)-1:"0000")
	sic_id$=callpoint!.getColumnData("SAM_CUSTSIC.SIC_CODE")
	prod$=callpoint!.getColumnData("SAM_CUSTSIC.PRODUCT_TYPE")
	item$=callpoint!.getColumnData("SAM_CUSTSIC.ITEM_ID")
	ltrip_key$=firm_id$+lyear$+sic_id$+prod$+item$
	sam_dev=fnget_dev("@SAM_CUSTSIC")
	dim sam_tpl$:fnget_tpl$("@SAM_CUSTSIC")
	dim qty[13],cost[13],sales[13]
	hshThisYear! = new java.util.HashMap()
	...
	tsls=0
	For x=1 to 13
		cst=num(callpoint!.getColumnData("SAM_CUSTSIC.TOTAL_COST_"+str(x:"00")))
		qty=num(callpoint!.getColumnData("SAM_CUSTSIC.QTY_SHIPPED_"+str(x:"00")))
		sls=num(callpoint!.getColumnData("SAM_CUSTSIC.TOTAL_SALES_"+str(x:"00")))
		tcst=tcst+cst
		tqty=tqty+qty
		tsls=tsls+sls
...
rem ========================================================
	...
	ctl1!=callpoint!.getControl("SAM_CUSTSIC.YEAR")
