...
rem --- Create totals
	...
	if cvs(callpoint!.getColumnData("SAM_CUSTOMER.YEAR"),3)<>""
...
rem --- Enable/Disable Summary button
	...
	cust_no$=callpoint!.getColumnData("SAM_CUSTOMER.CUSTOMER_ID")
	prod_type$=callpoint!.getColumnData("SAM_CUSTOMER.PRODUCT_TYPE")
	item_no$=callpoint!.getColumnData("SAM_CUSTOMER.ITEM_ID")
	gosub summ_button

[[SAM_CUSTOMER.AOPT-SUMM]]
rem --- Calculate and display summary info
	tcst=0
	tqty=0
	tsls=0
	year$=callpoint!.getColumnData("SAM_CUSTOMER.YEAR")
	lyear$=str(num(year$)-1:"0000")
	trip_key$=firm_id$+year$+callpoint!.getColumnData("SAM_CUSTOMER.CUSTOMER_ID")
	ltrip_key$=firm_id$+lyear$+callpoint!.getColumnData("SAM_CUSTOMER.CUSTOMER_ID")
	prod_type$=callpoint!.getColumnData("SAM_CUSTOMER.PRODUCT_TYPE")
	item_no$=callpoint!.getColumnData("SAM_CUSTOMER.ITEM_ID")
	if cvs(prod_type$,2)<>"" 
	...
	callpoint!.setColumnData("SAM_CUSTOMER.ITEM_ID","")
...
rem --- Now display all of these things and disable key fields
	for x=1 to 13
		callpoint!.setColumnData("SAM_CUSTOMER.TOTAL_SALES_"+str(x:"00"),str(sales[x]))
		callpoint!.setColumnData("SAM_CUSTOMER.TOTAL_COST_"+str(x:"00"),str(cost[x]))
		callpoint!.setColumnData("SAM_CUSTOMER.QTY_SHIPPED_"+str(x:"00"),str(qty[x]))
	next x
	...
	callpoint!.setColumnEnabled("SAM_CUSTOMER.YEAR",0)
	callpoint!.setColumnEnabled("SAM_CUSTOMER.CUSTOMER_ID",0)
	callpoint!.setColumnEnabled("SAM_CUSTOMER.PRODUCT_TYPE",0)
	callpoint!.setColumnEnabled("SAM_CUSTOMER.ITEM_ID",0)
...
[[SAM_CUSTOMER.AREC]]
rem --- Enable key fields, initialize totals, clear widget
	callpoint!.setColumnEnabled("SAM_CUSTOMER.YEAR",1)
	callpoint!.setColumnEnabled("SAM_CUSTOMER.CUSTOMER_ID",1)
	callpoint!.setColumnEnabled("SAM_CUSTOMER.PRODUCT_TYPE",1)
	callpoint!.setColumnEnabled("SAM_CUSTOMER.ITEM_ID",1)
...
rem --- Use current selections for initiating next record
	year$=callpoint!.getColumnData("SAM_CUSTOMER.YEAR")
	customer_id$=callpoint!.getColumnData("SAM_CUSTOMER.CUSTOMER_ID")
	product_type$=callpoint!.getColumnData("SAM_CUSTOMER.PRODUCT_TYPE")
	item_id$=callpoint!.getColumnData("SAM_CUSTOMER.ITEM_ID")
	sam_dev=fnget_dev("SAM_CUSTOMER")
	read(sam_dev,key=firm_id$+year$+customer_id$+product_type$+item_id$,dom=*next)
...
rem --- Use current selections for initiating previous record
	...
	item_id$=callpoint!.getColumnData("SAM_CUSTOMER.ITEM_ID")
	sam_dev=fnget_dev("SAM_CUSTOMER")
	read(sam_dev,key=firm_id$+year$+customer_id$+product_type$+item_id$,dir=0,dom=*next)
...
rem --- Check for parameter record
	...
	open_tables$[2]="SAM_CUSTOMER",open_opts$[2]="OTA@"
...
[[SAM_CUSTOMER.ITEM_ID.AINV]]
rem --- Item synonym processing

	call stbl("+DIR_PGM")+"ivc_itemsyn.aon::option_entry"

[[SAM_CUSTOMER.ITEM_ID.AVAL]]
rem --- Enable/Disable Summary button
	cust_no$=callpoint!.getColumnData("SAM_CUSTOMER.CUSTOMER_ID")
	prod_type$=callpoint!.getColumnData("SAM_CUSTOMER.PRODUCT_TYPE")
...
[[SAM_CUSTOMER.PRODUCT_TYPE.AVAL]]
rem --- Enable/Disable Summary button
	cust_no$=callpoint!.getColumnData("SAM_CUSTOMER.CUSTOMER_ID")
...
rem --- Use fiscal period and abbreviation for table row label
	...
	for i=1 to 13
		...
		ctlContext=num(callpoint!.getTableColumnAttribute("SAM_CUSTOMER.QTY_SHIPPED_"+period$,"CTLC"))
		ctlID=num(callpoint!.getTableColumnAttribute("SAM_CUSTOMER.QTY_SHIPPED_"+period$,"CTLI"))
...
rem =========================================================
	...
	lyear$=str(num(year$)-1:"0000")
	cust_no$=callpoint!.getColumnData("SAM_CUSTOMER.CUSTOMER_ID")
	prod$=callpoint!.getColumnData("SAM_CUSTOMER.PRODUCT_TYPE")
	item$=callpoint!.getColumnData("SAM_CUSTOMER.ITEM_ID")
	ltrip_key$=firm_id$+lyear$+cust_no$+prod$+item$
	sam_dev=fnget_dev("@SAM_CUSTOMER")
	dim sam_tpl$:fnget_tpl$("@SAM_CUSTOMER")
	dim qty[13],cost[13],sales[13]
	hshThisYear! = new java.util.HashMap()
	...
	tsls=0
	For x=1 to 13
		cst=num(callpoint!.getColumnData("SAM_CUSTOMER.TOTAL_COST_"+str(x:"00")))
		qty=num(callpoint!.getColumnData("SAM_CUSTOMER.QTY_SHIPPED_"+str(x:"00")))
		sls=num(callpoint!.getColumnData("SAM_CUSTOMER.TOTAL_SALES_"+str(x:"00")))
		tcst=tcst+cst
		tqty=tqty+qty
		tsls=tsls+sls
...
rem ========================================================
	...
	ctl1!=callpoint!.getControl("SAM_CUSTOMER.YEAR")
